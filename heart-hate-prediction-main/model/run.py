...
# 读取数据
data = pd.read_csv('C:/Users/dell/Desktop/pythonProject4/data/run.csv')  # 请替换为你的文件路径
...
# 设置时间步长，walk最佳值为7，jog最佳值为9,run最佳值为9
time_steps = 9
...
if X_train_scaled_seq.size == 0 or X_test_scaled_seq.size == 0:
    ...
    # 输出时间差
    time_diff = time_test.iloc[-1] - time_test.iloc[0]
    ...
    results_df.to_csv('C:/Users/dell/Desktop/pythonProject4/data/run results.csv', index=False)
    print("预测结果已保存至 run results.csv 文件")
