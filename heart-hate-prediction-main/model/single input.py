...
# 读取数据
data = pd.read_csv('C:/Users/dell/Desktop/pythonProject4/data/walk.csv')  # 替换为你的文件路径

# 选择单个特征和标签
feature = data[['Acceleration on chest']]  # 仅使用 一个加速度计 作为特征
...
filtered_feature = feature[condition]
filtered_labels = labels[condition]
filtered_time_column = time_column[condition]

# 重置索引
filtered_feature.reset_index(drop=True, inplace=True)
...
split_index = int(len(filtered_feature) * 0.9)
X_train, X_test = filtered_feature[:split_index], filtered_feature[split_index:]
...
if X_train_scaled_seq.size == 0 or X_test_scaled_seq.size == 0:
    ...
    time_test_seq = time_test[:min_len]  # 截取相同长度的时间序列

    # 输出时间差
    time_diff = time_test_seq.iloc[-1] - time_test_seq.iloc[0]
    print("Time difference between the first and last data points in the test set:", time_diff)
    # 保存真实值和预测值到 CSV 文件
